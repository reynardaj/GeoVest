import {
  PieChart, Pie, Cell, LineChart, Line, XAxis, YAxis, CartesianGrid, Tooltip, ResponsiveContainer, Legend, BarChart, Bar
} from 'recharts';
...
type RegionKey = "JAKARTA BARAT" | "JAKARTA PUSAT" | "JAKARTA SELATAN" | "JAKARTA TIMUR" | "JAKARTA UTARA";
...
const getAgeData = (region: string) => {
  ...
  return Object.entries(AgeDistribution[formattedRegion]).map(([name, value]) => ({
    name,
    value
  }));
};
...
const getReligionData = (region: string) => {
  ...
  return Object.entries(ReligionDistribution[formattedRegion]).map(([name, value]) => ({
    name,
    value
  }));
};
...
const PropertyAnalytics = ({ selectedPropertyData, regionData }: PropertyAnalyticsProps) => {
  ...
  const COLORS_AGE = ['#97E6DC', '#76D7C4', '#4CB8C4', '#2F6F8F', '#2F4A6F', '#17488D', '#0F2C4D'];
  const COLORS_RELIGION = ['#97E6DC', '#4CB8C4', '#2F6F8F', '#2F4A6F', '#17488D', '#0F2C4D'];
  ...
  const hasDemographicData = (ageData.length > 0 || religionData.length > 0) && regionData?.regionName;

  if (!selectedPropertyData && !hasDemographicData) {
    return (
      <div className="px-4 py-8 text-center">
        <h2 className="text-xl font-bold text-[#17488D] mb-4">Proyeksi Properti</h2>
        <p className="text-gray-600">Pilih properti atau wilayah pada peta untuk melihat analitik.</p>
      </div>
    );
  }
    ...
    <div className="text-black space-y-6 px-4 py-2">
      ...
      {hasDemographicData && regionData?.regionName && (
        <div>
          {ageData.length > 0 && (
            <div className="mb-6">
              <h3 className="text-xl font-bold text-[#17488D] mb-8">Distribusi Umur {regionName || "Jakarta"}</h3>
                  ...
                  <PieChart>
                    ...
                    <Tooltip 
                      formatter={(value, name, props) => {
                        const total = ageData.reduce((sum, entry) => sum + entry.value, 0);
                        const percent = ((Number(value) / total) * 100).toFixed(1);
                        return [`${Number(value).toLocaleString("id-ID")} (${percent}%)`, name];
                      }}
                      itemStyle={{
                        color: "#17488D"
                      }}  
          ...
          {religionData.length > 0 && (
            <div>
              <h3 className="text-xl font-bold text-[#17488D] mb-2">Distribusi Agama {regionName || "Jakarta"}</h3>
              <div className="h-[350px] w-full mt-6">
                <ResponsiveContainer width="100%" height="100%">
                  <PieChart>
                    ...
                    <Tooltip 
                      formatter={(value, name, props) => {
                        const total = religionData.reduce((sum, entry) => sum + entry.value, 0);
                        const percent = ((Number(value) / total) * 100).toFixed(1);
                        return [`${Number(value).toLocaleString("id-ID")} (${percent}%)`, name];
                      }}
                      itemStyle={{
