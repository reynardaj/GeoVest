...
export function useMapEvents(
  ...
  useEffect(() => {
    ...
    // --- Region Click ---
    const handleRegionClick = (e: MapMouseEvent & { features?: MapGeoJSONFeature[] }) => {
    ...
    // --- Properties Click ---
    const handlePropertyClick = (e: MapMouseEvent & { features?: MapGeoJSONFeature[] }) => {
        const feature = e.features?.[0];
        if (!feature) return;
        e.originalEvent.preventDefault(); // Prevent background click
    ...
    // --- Region Mousemove ---
    const handleRegionMouseMove = (e: MapMouseEvent & { features?: MapGeoJSONFeature[] }) => {
        if (e.features?.length) {
            const featureId = e.features[0].id;
            if (featureId !== undefined && featureId !== hoveredRegionIdRef.current) {
                // Reset previous hover state
                if (hoveredRegionIdRef.current !== null) {
                    map.setFeatureState({ source: 'jakarta', id: hoveredRegionIdRef.current }, { hover: false });
                    handlersRef.current.onRegionHover?.(null, map); // Notify parent: hover ended
                }
                // Set new hover state
                hoveredRegionIdRef.current = featureId;
                map.setFeatureState({ source: 'jakarta', id: featureId }, { hover: true });
    ...
    const handlePropertyMouseMove = (e: MapMouseEvent & { features?: MapGeoJSONFeature[] }) => {
        if (e.features?.length) {
            const featureId = e.features[0].id ?? e.features[0].properties?.id; // Use generated ID first
            if (featureId !== undefined && featureId !== hoveredPropertyIdRef.current) {
                 if (hoveredPropertyIdRef.current !== null) {
                    map.setFeatureState({ source: 'properties', id: hoveredPropertyIdRef.current }, { hover: false });
                    handlersRef.current.onPropertyHover?.(null, map);
                }
                hoveredPropertyIdRef.current = featureId;
                map.setFeatureState({ source: 'properties', id: featureId }, { hover: true });
